"""Unit and integration tests for MAPEOGEO v0.15.2 Confirmatory Real Analysis expansion."""
...
from scripts.analysis_intake_v0_15_2 import (
    compute_v0_15_2_metrics,
    ingest_analysis_declarations,
    ingest_v0_15_2_alignments,
...
OUTPUT_DIR = ROOT / "artifacts" / "analysis_v0_15_2"
...
def test_alignments_schema_v0_15_2():
...
def test_confirmatory_analysis_intake_pipeline(tmp_path: Path):
    ...
    out_dir = tmp_path / "artifacts" / "analysis_v0_15_2"
    graph, metrics, summary = run_analysis_intake_v0_15_2(
    ...
    assert metrics["source_breakdown"]["S_A_gallier"] == len(gallier)
    assert metrics["source_breakdown"]["S_B_axler"] == len(axler)
    assert metrics["source_breakdown"]["S_C_vmls"] == len(vmls)
    assert metrics["source_breakdown"]["S_D_cvx"] == len(cvx)

    # Invariant: Namespace consistency
    for n in axler:
        assert "axler" in n["id"], f"Namespace error: {n['id']}"
    for n in vmls:
        assert "vmls" in n["id"], f"Namespace error: {n['id']}"
    for n in cvx:
        assert "cvx" in n["id"], f"Namespace error: {n['id']}"
    ...
    out_graph = out_dir / "mapeogeo_v0_15_2_graph.json.gz"
