"""Unit and integration tests for MAPEOGEO v0.15 Real Analysis and Differential Calculus expansion."""
...
from scripts.analysis_intake_v0_15 import (
    compute_v0_15_metrics,
...
def test_alignments_schema_v0_15():
...
def test_analysis_intake_pipeline(tmp_path: Path):
    ...
    assert metrics["edges_summary"]["SAME_SEMANTICS_bridges"] >= 250
    assert metrics["edges_summary"]["SCOPED_OVERLAP_bridges"] >= 20
    assert metrics["edges_summary"]["total_cross_source_bridges"] >= 300

    out_graph = out_dir / "mapeogeo_v0_15_graph.json.gz"
