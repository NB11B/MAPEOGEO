#!/usr/bin/env python3
"""MAPEOGEO v0.15 Real Analysis and Differential Calculus Expansion Validator.
...
5. Representation Diversity Profile R(M) and richness r_bar >= 2.75.
6. Domain coverage spanning at least 4 distinct mathematical domains.
7. Typed semantic bridge edges (SAME_SEMANTICS, SCOPED_OVERLAP, RELATED_TO).
8. Formal proof bindings (Lean 4 kernel-verified theorems).
9. Strict zero-prose persistence across all graph nodes.
"""
...
from scripts.import_analysis_v0_15 import FORBIDDEN_PERSISTED_KEYS

DEFAULT_GRAPH_PATH = ROOT / "artifacts" / "analysis_v0_15" / "mapeogeo_v0_15_graph.json.gz"
DEFAULT_DASHBOARD_PATH = ROOT / "artifacts" / "analysis_v0_15" / "analysis_expansion_dashboard.json"
DEFAULT_PREREG_PATH = ROOT / "evidence" / "v0_15_preregistration.json"
...
def validate_v0_15(
    ...
    print(f"=== Validating MAPEOGEO v0.15 Analysis and Differential Calculus Expansion ===")
    errors: list[str] = []
    ...
    if len(gallier_decls) < 25:
        errors.append(f"Insufficient Gallier declarations: {len(gallier_decls)} < 25")
    ...
    min_source_total = prereg.get("min_total_source_declarations", 400)
    ...
    min_canonical = prereg.get("min_canonical_objects", 85)
    ...
    min_2_source = prereg.get("min_two_source_supported", 45)
    ...
    min_3_source = prereg.get("min_three_source_supported", 30)
    ...
    min_domains = prereg.get("min_domains", 4)
    ...
    min_r_bar = prereg.get("min_average_representation_richness", 2.75)
    ...
    min_formal = prereg.get("min_formal_proof_linked", 6)
    if formal_linked < min_formal:
        errors.append(f"Formal proof linked canonical objects {formal_linked} < {min_formal}")
    ...
    if same_sem < 250:
        errors.append(f"SAME_SEMANTICS bridge edges {same_sem} < 250")
    if scoped_ov < 20:
        errors.append(f"SCOPED_OVERLAP bridge edges {scoped_ov} < 20")
    if tot_bridges < 300:
        errors.append(f"Total cross-source bridge edges {tot_bridges} < 300")
    ...
    if errors:
        ...
        print("\nALL v0.15 ANALYSIS EXPANSION VALIDATION CHECKS PASSED!")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate MAPEOGEO v0.15 Analysis Expansion")
