...
plain_graph = ROOT / "data" / "gallier_quaintance_graph_v0_3.json"
gzip_graph = ROOT / "data" / "gallier_quaintance_graph_v0_3.json.gz"

if plain_graph.exists():
    graph = json.loads(plain_graph.read_text(encoding="utf-8"))
elif gzip_graph.exists():
    graph = json.loads(gzip.decompress(gzip_graph.read_bytes()).decode("utf-8"))
else:
    raise FileNotFoundError("missing Gallier/Quaintance v0.3 graph artifact")
