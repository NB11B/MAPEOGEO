...
contracts.py, graph.py, factorization.py, rewrites.py, receipt_checker.py, scheduler.py, solver_adapter.py, deterministic_backend.py, local_model_backend.py, fixtures.py, metrics.py, campaign.py, cli.py and test_*.py.
...
B3 custom parallel sampler work is not part of the v1.1 primary implementation.
...
If B2 is genuinely implemented and preflight validates shared-state/prefix reuse, add B2 explicitly. The CLI must reject a requested backend level not supported by the sealed BackendDescriptor.
...
2. the user/runtime selects a supported backend level and resource budget;
3. seal hashes the BackendDescriptor, CostProfile, ResourceBudget, fixtures, factorization registry, policy and environment into RunLock;
4. prepare(run_lock, expected_descriptor_hash) reprobes/validates the live backend before execution and refuses drift.

Seal generation must refuse absent pins, missing fixtures, missing FactorizationContracts, missing local model files for model runs, unsupported requested capabilities, dirty implementation trees for confirmatory runs and unresolved environment drift. It must never insert fake model hashes or reuse this documentation's base commit as the implementation commit.
...
- option-order association correctness;
- no ACTION emission from the backend.

B1:

- same per-question query/state identity as B0;
- batch permutations satisfy the preregistered drift gate;
...
9. Add B1 batching and validate isolated-vs-batched invariance.
10. Add B2 only if shared-state reuse is real and measurable.
11. Freeze the complete implementation, data, factorization contracts, costs and environment; run the full matrix once.
12. Publish raw failures and bounded conclusions before deciding whether a new campaign version is justified.
