...
theorem SupInfDef (s : Set ℝ) (hnonempty : s.Nonempty) (hbd : BddAbove s) (eps : ℝ) (heps : 0 < eps) :
    ∃ x ∈ s, sSup s - eps < x := by
  exact exists_lt_of_lt_csSup hnonempty (sub_lt_self (sSup s) heps)

-- 4. Least Upper Bound Axiom / Property (Lebl §1.2 Axiom 1.2.3)
theorem LeastUpperBoundAxiom (s : Set ℝ) (hnonempty : s.Nonempty) (hbd : BddAbove s) :
    ∀ x ∈ s, x ≤ sSup s := by
  intro x hx
...
theorem ArchimedeanProperty (x : ℝ) (eps : ℝ) (heps : 0 < eps) :
  ...
  · obtain ⟨n, hn⟩ := exists_nat_gt (1 / eps)
    have hn_pos : 0 < n := by
      by_contra hc
      have hle : n = 0 := Nat.eq_zero_of_nonpos_iff.2 (not_lt.1 hc)
      subst hle
...
theorem NestedIntervalProperty (a b : ℕ → ℝ) (h_nested : ∀ n, a n ≤ a (n + 1) ∧ b (n + 1) ≤ b n)
  ...
  have h_bdd_a : BddAbove (range a) := by
    use b 0
    rintro _ ⟨n, rfl⟩
    exact le_trans (h_le n) (antitone_nat_of_succ_le (fun m => (h_nested m).2) (zero_le n))
  use sSup (range a)
  intro n
  constructor
  ...
  · apply csSup_le ⟨a 0, mem_range_self 0⟩
    ...
    · exact le_trans (h_le m) (antitone_nat_of_succ_le (fun k => (h_nested k).2) hnm)
...
theorem CauchyCompleteness (u : ℕ → ℝ) :
  ...
  exact cauchySeq_iff_exists_tendsto
...
theorem MonotoneConvergence (u : ℕ → ℝ) (hmono : Monotone u) (hbd : BddAbove (range u)) :
  ...
  exact tendsto_atTop_csSup hmono hbd
...
theorem BolzanoWeierstrass (u : ℕ → ℝ) (hbd : BddAbove (range u)) (hbd_below : BddBelow (range u)) :
  ...
  have h_compact : IsCompact (Icc (sInf (range u)) (sSup (range u))) := isCompact_Icc
  have h_in : ∀ n, u n ∈ Icc (sInf (range u)) (sSup (range u)) := fun n =>
    ⟨csInf_le hbd_below (mem_range_self n), le_csSup hbd (mem_range_self n)⟩
  obtain ⟨L, -, φ, hφ, hconv⟩ := isCompact_iff_seq_compact.mp h_compact (fun n => u n) (fun n => h_in n)
  exact ⟨φ, hφ, L, hconv⟩
...
theorem HeineBorel (s : Set ℝ) :
    IsCompact s ↔ IsClosed s ∧ Bounded s := by
  exact isCompact_iff_isClosed_bounded

-- 14. Sequential Continuity Equivalence (Lebl §3.3 Prop 3.3.2)
theorem SequentialContinuity (f : ℝ → ℝ) (x0 : ℝ) :
    ContinuousAt f x0 ↔ ∀ u : ℕ → ℝ, Tendsto u atTop (𝓝 x0) → Tendsto (f ∘ u) atTop (𝓝 (f x0)) := by
  exact continuousAt_iff_seq_continuousAt
...
theorem RollesTheorem (a b : ℝ) (hab : a < b) (f : ℝ → ℝ)
  ...
  exact exists_deriv_eq_zero hab h_cont h_diff h_eq
...
theorem MeanValueTheorem (a b : ℝ) (hab : a < b) (f : ℝ → ℝ)
  ...
  exact ⟨c, hc, hderiv.symm⟩

-- 19. Fundamental Theorem of Calculus Part 1 (Lebl §5.3 Thm 5.3.1)
theorem FundamentalTheoremCalculus (a b : ℝ) (hab : a ≤ b) (f : ℝ → ℝ) (hf : ContinuousOn f (Icc a b))
    (x : ℝ) (hx : x ∈ Ico a b) :
    HasDerivWithinAt (fun y => ∫ t in a..y, f t) (f x) (Ici x) x := by
  exact integral_hasDerivWithinAt_right (hf x ⟨hx.1, le_of_lt hx.2⟩) hf (Ico_subset_Icc_self hx)
