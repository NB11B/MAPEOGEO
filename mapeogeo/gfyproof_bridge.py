...
BRIDGE_SCHEMA = "mapeogeo.gfyproof.edge-certificate.v1"
...
PROOF_FAMILY_EDGE_TYPES: dict[str, frozenset[str]] = {
    "E085_DFA_EQUIVALENCE": frozenset(
        {
            "REPRESENTS",
            "SAME_SEMANTICS",
        }
    ),
    "E091_ROBDD_EQUIVALENCE": frozenset(
        {
            "REPRESENTS",
            "SAME_SEMANTICS",
        }
    ),
    "E093_FARKAS_IMPLICATION": frozenset(
        {
            "UPWARD_FOUNDATION_DEPENDENCY",
            "PROOF_DEPENDENCY",
        }
    ),
    "E094_POLYNOMIAL_IDEAL": frozenset(
        {
            "UPWARD_FOUNDATION_DEPENDENCY",
            "PROOF_DEPENDENCY",
        }
...
def claim_contract_digest(
    *,
    ...
    return canonical_sha256(
        ...
        domain="mapeogeo-gfyproof-claim-v1",
    )
...
def gfyproof_edge_contract_digest(
    ...
    return hashlib.sha256(
        b"MAPEOGEO:GFYPROOF:edge-evidence:v1\0"
...
def validate_gfyproof_certificate(
    ...
    *,
    ...
    allowed = (
        PROOF_FAMILY_EDGE_TYPES.get(
            proof_family
        )
    )
    if allowed is None:
        raise GFYProofBridgeError(
            "unknown GFYProof proof family"
        )
    if edge_type not in allowed:
        raise GFYProofBridgeError(
            "proof family is not authoritative for this edge type"
        )
...
def materialize_gfyproof_edge(
    ...
    *,
    ...
    attributes: dict[
        ...
        "gfyproof_proof_family": validated[
            "proof_family"
        ],
