...
def main() -> int:
    ...
    # Stage: v0.21 Wave F1 (Logic, Set Theory & Discrete Mathematics)
    foundation_graph_path = ROOT / "artifacts" / "foundation_backfill" / "mapeogeo_foundation_graph.json.gz"
    run_stage(
        "v0.21 Wave F1 Intake (Open Logic + Open Set Theory + Levin Discrete)",
        [
            sys.executable,
            str(ROOT / "scripts" / "wave_f1_intake.py"),
            "--base-graph",
            str(foundation_graph_path),
            "--out-dir",
            str(ROOT / "artifacts" / "wave_f1_v0_21"),
            "--evidence-out",
            str(ROOT / "evidence" / "v0_21_wave_f1_scientific_results.json"),
            "--report-out",
            str(ROOT / "docs" / "V0_21_WAVE_F1_REPORT.md"),
        ],
    )

    if args.target_stage == "wave_f1":
        total_time = time.time() - t_start
        print("==========================================================")
        print(f"  Clean-room reconstruction SUCCESS! Total time: {total_time:.2f}s")
        print("==========================================================")
    ...
    # Stage: v0.21 Wave F2 (EO/GEO Dual-View Realization and Commutation Audit)
    run_stage(
        "v0.21 Wave F2 (EO/GEO Dual-View Realization & Commutation Audit)",
        [
            sys.executable,
            str(ROOT / "scripts" / "wave_f2_commutation_campaign.py"),
            "--manifest",
    ...
    # Stage: v0.21 Wave F4 (Quantified Real Analysis and Proof-Bearing Dual Views)
    run_stage(
        "v0.21 Wave F4 (Quantified Real Analysis and Proof-Bearing Dual Views)",
        [
            sys.executable,
            str(ROOT / "scripts" / "wave_f4_campaign.py"),
            "--out-evidence",
