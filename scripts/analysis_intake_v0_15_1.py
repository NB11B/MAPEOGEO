#!/usr/bin/env python3
"""MAPEOGEO v0.15.1 Confirmatory Real Analysis and Differential Calculus Expansion Runner.
...
Dashboard Metrics:
- N_source_total: Total source declarations across S_A, S_B, S_C, S_D
...
def compute_v0_15_1_metrics(graph: dict[str, Any], alignment_summary: dict[str, Any]) -> dict[str, Any]:
...
def run_analysis_intake_v0_15_1(
    base_graph_path: Path,
    alignments_path: Path,
    out_dir: Path,
) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    print(f"=== Running MAPEOGEO {STAGE} Confirmatory Analysis and Differential Calculus Expansion ===")
    ...
    # 5. Compute metrics
    metrics = compute_v0_15_1_metrics(graph, alignment_summary)

    # 6. Save graph artifact
    out_graph_gz = out_dir / "mapeogeo_v0_15_1_graph.json.gz"
    ...
    scientific_results_path = evidence_dir / "v0_15_1_scientific_results.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run MAPEOGEO v0.15.1 Confirmatory Real Analysis Expansion")
    parser.add_argument(
        "--base-graph",
        type=Path,
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "artifacts" / "analysis_v0_15_1",
        help="Output directory for v0.15.1 artifacts",
    )
